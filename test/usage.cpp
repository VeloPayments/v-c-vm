...
TEST(simple_build)
{
    ...
    bytecode_builder_context_t builder;
    bytecode_builder_init(&builder, &allocator);

    bytecode_builder_add_instruction(&builder, opcode_pack_one_arg(OPCODE_LOAD_CONST_STRING, bytecode_builder_add_string(&builder, const_cast<char*>("testing"))));
